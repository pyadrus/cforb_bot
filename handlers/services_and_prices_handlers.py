...
from keyboards.user_keyboards import services_and_prices_key, keyboard_to_main_menu
...
async def white_cargo_delivery_with_gas_turbine_engine(callback_query: types.CallbackQuery, state: FSMContext):
    try:
        ...
        await state.reset_state()  # Сбрасываем все данные машины состояний, до значения по умолчанию
        greeting_message = (
            f"<b>Поможем перевезти груз и пройти таможню без потерь времени и денег. Сделаем комплексную поставку: подберем транспорт, соберем документы, подадим декларацию, оформим и привезем груз на склад. Или подключимся на нужном этапе.</b>\n\n"
...
async def supplier_inspection_by_province(callback_query: types.CallbackQuery, state: FSMContext):
    try:
        ...
        await state.reset_state()  # Сбрасываем все данные машины состояний, до значения по умолчанию
        greeting_message = (
            f"✅ Инспекция поставщика по провинциям 1000-1500 юаней (по актуальному курсу) плюс транспортные расходы, по необходимости жильё.\n"
            f"Что входит?!\n"
...
async def how_is_payment_made(callback_query: types.CallbackQuery, state: FSMContext):
    try:
        ...
        await state.reset_state()  # Сбрасываем все данные машины состояний, до значения по умолчанию
        greeting_message = (
            f"Вы можете выбрать удобный и необходимый для Вас способ (Здесь указана схема оплаты только по Карго перевозкам, по работе в Белую и Белой доставке необходимо связываться со специалистом ВЭД).\n"
            f"Оплата за товар:\n"
