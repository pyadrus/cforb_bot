...
async def white_cargo_delivery_with_gas_turbine_engine(callback_query: types.CallbackQuery, state: FSMContext):
    try:
        await state.finish()  # Завершаем текущее состояние машины состояний
        await state.reset_state()  # Сбрасываем все данные машины состояний, до значения по умолчанию
        greeting_message = (f"<b>Поможем перевезти груз и пройти таможню без потерь времени и денег. Сделаем комплексную поставку: подберем транспорт, соберем документы, подадим декларацию, оформим и привезем груз на склад. Или подключимся на нужном этапе.</b>\n\n"
...
async def product_search_service(callback_query: types.CallbackQuery, state: FSMContext):
    try:
        ...
        greeting_message = (f"✅ Тарифы на поиск:\n"
                            ...
                            f"Срок готовности ответа на запрос поиска, от 3 до 8 рабочих дней (в течение этого времени можем уточнять у Вас детали касающиеся товара, к примеру, если это фабрика, то товар можно будет изготовить прямо под Ваше техзадание).\n"
                            f"Не берем на поиск более двух товарных позиций за один раз, ищем 1-2 позиции, начинаем сотрудничество по ним, далее переходим к поиску других товаров.\n"
                            f"Мы берём с Вас гарантийный платёж в размере: 1500₽\n"
                            f"Для чего это нужно?\n"
                            f"·Гарантийный платёж является нашим вознаграждением за потраченное время на поиски товара и общение с поставщиками в том случае, если Вы передумаете. Ведь в это время мы могли бы помочь другому клиенту с поиском и выкупом товара.\n"
                            f"Что дальше?\n"
                            f"· По результатам поиска мы предоставляем Вам подборку из лучших предложений (1-3 варианта).\n"
                            f"· Если Вы оформляете заказ, то сумма гарантийного платежа учитывается в стоимость оказанных услуг (таким образом, вычитаем из суммы за услуги внесённый ранее гарантийный платёж.).\n"
                            ...
                            f"· Доставка товара оплачивается отдельно, исходя из категории товара, определяется тариф на логистику.\n"
                            f"📦 Проверка товара на брак и на количество:\n"
                            f"Одна товарная позиция - одна единица товара фотоотчет, видеоотчет, осуществляется бесплатно по запросу. Всё что необходимо свыше, обговаривается индивидуально.\n"
                            f"✅ Дополнительная услуга: наклейки со штрих кодами на товар для маркетплейсов от 100-300 шт 1 юань (по курсу).\n"
                            f"✅ Дополнительная услуга: проверка количества товара на соответствие, 1-2 юаня за 1 счетную единицу (зависит от упаковки товара)\n"
                            f"Связаться с менеджерами: @cargo_cfb")
        await bot.send_message(callback_query.from_user.id,  # ID пользователя
                               text=greeting_message,  # Текст для приветствия 👋
                               parse_mode=types.ParseMode.HTML)  # Текст в HTML-разметки
...
async def supplier_inspection_by_province(callback_query: types.CallbackQuery, state: FSMContext):
    try:
        await state.finish()  # Завершаем текущее состояние машины состояний
        await state.reset_state()  # Сбрасываем все данные машины состояний, до значения по умолчанию
        greeting_message = (f"✅ Инспекция поставщика по провинциям 1000-1500 юаней (по актуальному курсу) плюс транспортные расходы, по необходимости жильё.\n"
                            f"Что входит?!\n"
                            ...
                            f"Оправка каталогов, только авиа. Оплата за доставку по факту прихода посылки в Москву. Также возможна пересылка по городу или в другие регионы.\n"
...
async def wechat_registration_service(callback_query: types.CallbackQuery, state: FSMContext):
    try:
        await state.finish()  # Завершаем текущее состояние машины состояний
        await state.reset_state()  # Сбрасываем все данные машины состояний, до значения по умолчанию
        greeting_message = (f"Мы готовы помочь вам с регистрацией вашего WeChat аккаунта. Отсканируем ваш QR-код мгновенно.\n"
                            f"Стоимость услуги 2000₽\n"
                            f"Регистрация в WeChat действует по принципу «Приглашение по поручительству».\n"
                            f"После прохождения всех процедур, в заключительной стадии верификации WeChat, появляется картинка с QR-кодом. Именно этот код необходимо выслать действующему пользователю, чтобы он в разделе «Отсканировать QR-код» подтвердил бы Ваше намерение стать новым пользователем, и желательно в пределах 1 минуты после получения QR.\n"
...
async def purchase_a_supplier_database(callback_query: types.CallbackQuery, state: FSMContext):
    try:
        ...
        greeting_message = (f"У нас вы можете приобрести две разные базы поставщиков:\n"
                            ...
                            f"• Десятки контактов фабрик и поставщиков лично собранные нашей компанией на выставках и ярмарках.\n"
                            f"Цена 1999 руб.\n"
                            f"2. База Ночных рынков включает в себя:\n"
                            f"• Сотни контактов поставщиков копий брендов: одежда, обувь, сумки и т.д. (Собрана нашей компанией при личном посещение торговых точек)\n"
                            f"Цена 4999 руб.\n"
                            f"Связаться с менеджерами: @cargo_cfb\n")
        await bot.send_message(callback_query.from_user.id,  # ID пользователя
                               text=greeting_message,  # Текст для приветствия 👋
...
async def what_payments_await_me(callback_query: types.CallbackQuery, state: FSMContext):
    try:
        await state.finish()  # Завершаем текущее состояние машины состояний
        await state.reset_state()  # Сбрасываем все данные машины состояний, до значения по умолчанию
        greeting_message = (f"В нашей компании нет никаких скрытых платежей, мы максимально честны и открыты перед нашими клиентами.\n"
                            f"Чтобы не было недопониманий, мы полностью расскажем за что приходится платить при карго доставке.\n"
                            f"• Оплата самого товара и доставки по Китаю к нам на склад ( цена доставки зависит от поставщика, многие доставляют бесплатно).\n"
                            ...
                            f"• Погрузочно-разгрузочные работы и хранение груза если не забираете груз в день прибытия и оставляете на следующий день в Москве/Алматы по местным тарифам.\n"
...
async def how_is_payment_made(callback_query: types.CallbackQuery, state: FSMContext):
    try:
        await state.finish()  # Завершаем текущее состояние машины состояний
        await state.reset_state()  # Сбрасываем все данные машины состояний, до значения по умолчанию
        greeting_message = (f"Вы можете выбрать удобный и необходимый для Вас способ (Здесь указана схема оплаты только по Карго перевозкам, по работе в Белую и Белой доставке необходимо связываться со специалистом ВЭД).\n"
                            f"Оплата за товар:\n"
                            f"• Оплата рублями на банковские карты РФ (Сбербанк, Тинькофф)\n"
                            f"• Мы можем помочь вам и принять оплату на Р/C - при оплате на расчётный счёт, +10% к общей сумме. При данном виде оплаты работаем по договору, с предоставлением ТОРГ-12, минимальный вес грузов к перевозке при таком расчете - 400кг.\n"
