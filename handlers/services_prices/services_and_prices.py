...
async def replace_photo(message: types.Message):
    ...
    await message.bot.download_file(file_info.file_path, os.path.join("media/photos/", 'services_and_prices.jpg'))
    ...
    try:
        ...
        await bot.edit_message_media(
            media=InputMediaPhoto(media=FSInputFile('media/photos/services_and_prices.jpg'),
                                  caption=load_bot_info(messages="media/messages/services_prices_messages/services_and_prices.json"),
                                  parse_mode="HTML"),
...
async def cargo_delivery_prices(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    try:
        await state.clear()  # Очищаем состояние
        await bot.send_document(chat_id=callback_query.message.chat.id, document=FSInputFile('media/document/Прейскурант CFORB.xlsx'),
...
async def update_info(message: Message, state: FSMContext):
    save_bot_info(message.html_text, file_path='media/messages/cargo_delivery_prices.json')  # Сохраняем информацию в JSON
    await message.reply("Информация обновлена.")
    await state.clear()
...
async def update_info(message: Message, state: FSMContext):
    save_bot_info(message.html_text, file_path='media/messages/goods_redemption_service.json')  # Сохраняем информацию в JSON
    await message.reply("Информация обновлена.")
    await state.clear()
...
async def update_info(message: Message, state: FSMContext):
    save_bot_info(message.html_text, file_path='media/messages/product_search_service.json')  # Сохраняем информацию в JSON
    await message.reply("Информация обновлена.")
    await state.clear()
...
async def update_info(message: Message, state: FSMContext):
    save_bot_info(message.html_text, file_path='media/messages/wechat_registration_service.json')  # Сохраняем информацию в JSON
    await message.reply("Информация обновлена.")
    await state.clear()
...
async def update_info(message: Message, state: FSMContext):
    save_bot_info(message.html_text, file_path='media/messages/purchase_a_supplier_database.json')  # Сохраняем информацию в JSON
    await message.reply("Информация обновлена.")
    await state.clear()
...
async def update_info(message: Message, state: FSMContext):
    save_bot_info(message.html_text, file_path='media/messages/what_payments_await_me.json')  # Сохраняем информацию в JSON
    await message.reply("Информация обновлена.")
    await state.clear()
