...
async def bag_tape(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        greeting_message = (f"Вставка видео https://youtube.com/shorts/E2LFiy8iF0g упаковки и текст:\n\n"
                            f"<b>Мешок + скотч:</b> классические упаковка с использованием мешка и упаковочного скотча, подходит для текстиля и мягких товаров.\n\n"
...
async def box_bag_tape(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        greeting_message = (
            ...
            f"<b>Кробка + мешок + скотч:</b> если у вас много разных товаров в том числе маленькие коробочки, весь товар складывается в нашу коробку, далее упаковка в мешок и упаковочный скотч.\n\n"
...
async def wooden_sheathing_bag_tape(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        greeting_message = (
            ...
            f"<b>Деревянная обрешетка + мешок + скотч:</b> Каркас из деревянных решеток (досок, брусьев), который способствует хорошей фиксации груза и его защите от механических повреждений. Применяется для упаковки хрупкого и бьющегося товара. + мешок + скотч\n\n"
...
async def wooden_corners_bag_tape(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        greeting_message = (
            ...
            f"<b>Уголки + мешок + скотч:</b> Легче чем обрешетка, держит форму коробки, но менее защищенная, т.к практически полностью открытая.\n\n"
            f"<b>Стоимость данной упаковки 10$ место</b>\n\n"
            f"<b>Связаться с менеджерами: @cargo_cfb</b>\n\n")
        types_of_packaging_key = types_of_packaging_keyboard_menu()
        await bot.send_message(callback_query.from_user.id,  # ID пользователя
...
async def pallet_in_crate(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        greeting_message = (
            f"Вставка видео https://youtube.com/shorts/QcXqjaESW7s упаковки и текст:\n\n"
            f"<b>Паллет в обрешетке:</b> Вид упаковки используется для крупногабаритных или хрупких грузов, представляет из себя поддон с деревянными бортами как у обрешетки. Погрузочно-разгрузочные работы товаров, упакованных таким образом, осуществляются при помощи вилочного погрузчика.\n\n"
...
async def pallet_with_a_solid_wooden_box(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await state.reset_state()  # Сбрасываем все данные машины состояний, до значения по умолчанию
        greeting_message = (
            f"<b>Паллет с глухим деревянным коробом:</b> Вид упаковки используется для крупногабаритных или хрупких грузов, представляет из себя поддон с деревянными бортами как у обрешетки. Погрузочно-разгрузочные работы товаров, упакованных таким образом, осуществляются при помощи вилочного погрузчика.\n\n"
            f"Фото отсутствует\n\n"
